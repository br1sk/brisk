...
class FileDroppableView: NSView {
    // AnyObject is a workaround for XCode8 InterfaceBuilder not supporting Swift protocols as IBOutlets
    @IBOutlet private weak var dropDelegate: AnyObject?
    ...
    override func draggingEntered(_ sender: NSDraggingInfo) -> NSDragOperation {
        guard let dropDelegate = dropDelegate as? FileDroppableViewDelegate,
            let files = filesfromDraggingInfo(sender) else {
            return []
        }
